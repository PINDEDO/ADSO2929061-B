...
<body class="bg-cyan-700 min-h-dvh p-14">
    <h1 class="text-bg-cyan-200 text-4xl text-center border-b-3 pb-4">Get All Pets (View)</h1>
    <table class="table table-border bg-cyan-300 mt-5 text-bg-cyan-950">
        <thead>
        ...
        <tbody>
            @foreach ($pets as $pet)
                <tr class="even:bg-cyan-400">
                    ...
                    <td>{{$pet->breed}}</td>
                    <td>
                        <a href=" {{url('show/pet/'.$pet->id) }}" class="bg-cyan-600 flex p-2 text-bg-cyan-200 w-10 rounded-lg justify-center scale-90 hover:scale-100 transition-all">
